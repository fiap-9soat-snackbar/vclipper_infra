...
resource "aws_cloudwatch_dashboard" "vclipper_infrastructure" {
  dashboard_name = "VClipper-Infrastructure-${var.environment}"
  ...
  dashboard_body = templatefile("${path.module}/dashboards/infrastructure_dashboard.json", {
    ...
    queue_name              = data.terraform_remote_state.sqs_processing.outputs.queue_name
    dlq_name                = data.terraform_remote_state.sqs_processing.outputs.dlq_name
